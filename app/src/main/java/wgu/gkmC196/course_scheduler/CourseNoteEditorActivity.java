...
public class CourseNoteEditorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(wgu.gkmC196.course_scheduler.R.layout.activity_course_note_editor);
        Toolbar toolbar = (Toolbar) findViewById(wgu.gkmC196.course_scheduler.R.id.toolbar);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        noteTextField = (EditText) findViewById(wgu.gkmC196.course_scheduler.R.id.etCourseNoteText);
        courseNoteUri = getIntent().getParcelableExtra(DataProvider.COURSE_NOTE_CONTENT_TYPE);

        if (courseNoteUri == null) {
            setTitle(getString(wgu.gkmC196.course_scheduler.R.string.enter_new_note));
            ...
            setTitle(getString(wgu.gkmC196.course_scheduler.R.string.edit_note));
