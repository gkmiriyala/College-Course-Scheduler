...
public class TermEditorActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(wgu.gkmC196.course_scheduler.R.layout.activity_term_editor);
        ...
        termNameField = (EditText) findViewById(wgu.gkmC196.course_scheduler.R.id.termNameEditText);
        termStartDateField = (EditText) findViewById(wgu.gkmC196.course_scheduler.R.id.termStartDateEditText);
        termStartDateField.setInputType(InputType.TYPE_NULL);
        termEndDateField = (EditText) findViewById(wgu.gkmC196.course_scheduler.R.id.termEndDateEditText);
        ...
        if (uri == null) {
            ...
            setTitle(getString(wgu.gkmC196.course_scheduler.R.string.add_new_term));
        }
        else {
            action = Intent.ACTION_EDIT;
            setTitle(getString(wgu.gkmC196.course_scheduler.R.string.edit_term_title));
    ...
    public void saveTermChanges(View view) {
        if (action == Intent.ACTION_INSERT) {
            ...
            Toast.makeText(this, getString(wgu.gkmC196.course_scheduler.R.string.term_saved), Toast.LENGTH_SHORT).show();
            setResult(RESULT_OK);
        }
        else if (action == Intent.ACTION_EDIT) {
            getTermFromForm();
            term.saveChanges(this);
            Toast.makeText(this, getString(wgu.gkmC196.course_scheduler.R.string.term_updated), Toast.LENGTH_SHORT).show();
            setResult(RESULT_OK);
        }
        finish();
    }
